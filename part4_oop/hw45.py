...
class LFUPolicy(Policy[K]):
    ...
    def get_key_to_evict(self) -> K | None:
        if len(self._key_counter) > self.capacity:
            keys_to_consider = list(self._key_counter.keys())
            keys_to_consider.pop()
            if not keys_to_consider:
                return None
            return min(
                keys_to_consider,
                key=lambda k: self._key_counter[k],
            )
        return None
...
class CachedProperty[V]:
    ...
    def __get__(
        ...
        cached_value = cache.get(self.name)

        if cached_value is not None:
            return cast("V", cached_value)

        computed_value = self.func(instance)
        cache.set(self.name, computed_value)
